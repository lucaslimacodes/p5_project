class Astar extends Algorithm{
    constructor(grid, startPosition, endPosition){
        ...
        this.frontier.enqueue(startPosition, this.grid[startPosition.x][startPosition.y] + this.heuristic(startPosition))
    }
    ...
    runStep(){
        ...
        for(let i=0;i<neighbors.length;i++){
            ...
            if(this.isPositionInFrontier(neighbors[i]) == false && this.wasPositionExplored(neighbors[i]) == false){
                let g_cost = state.priority - this.heuristic(state.value)
                newPriority = g_cost + this.grid[neighbors[i].x][neighbors[i].y] + this.heuristic(neighbors[i])
